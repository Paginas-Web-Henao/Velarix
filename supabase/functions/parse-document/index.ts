...
import { normalizarTexto, extraerCSV, type ParsedRow } from "../_shared/parse-document-csv-extractor.ts";
...
// ── Text Normalization + CSV Extraction ──
// normalizarTexto()/extraerCSV()/ParsedRow viven en
// ../_shared/parse-document-csv-extractor.ts (reubicadas desde aquí para
// que la detección de encabezado quede cubierta por tests) — importadas
// arriba.
...
// CSV Extraction
// ═══════════════════════════════════════════════════════════════
// extraerCSV() vive en ../_shared/parse-document-csv-extractor.ts —
// importada arriba.
...
serve(async (req) => {
  ...
  try {
    ...
    const mapped = mapErrorToResponse(error, { code: "PARSE_ERROR", message: "Error al procesar el documento." });
