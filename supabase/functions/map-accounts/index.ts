...
serve(async (req) => {
  ...
  try {
    ...
    const { data: analysis, error: analysisError } = await supabase.from("analyses").select("id, user_id").eq("id", analysis_id).single();
    ...
    const mapped = mapErrorToResponse(error, { code: "MAPPING_ERROR", message: "Error en la homologación contable." });
