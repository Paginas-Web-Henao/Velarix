...
| Alcance real verificado (2026-08-06) | **La política aplica sin excepción al flujo profesional real**: `NewAnalysisStepper.tsx` no expone `equityWeight`/`debtWeight` en absoluto — confirmado por `grep`, cero resultados — y el servidor (`ejecutar-calculo`) siempre deriva la estructura de saldos reales, sin excepción. **La política todavía NO aplica al demo/estimador**: `DemoInputsForm.tsx` (`src/components/demo/DemoInputsForm.tsx:169-181`) sigue exponiendo `equityWeight` como campo editable (`debtWeight` se deriva automáticamente como `1 - equityWeight`) — no se tocó en `c6619d5` porque pertenece a esta Decisión 2, no a la Decisión 9 (que solo autorizaba retirar los 7 supuestos de `_shared/financial-methodology.ts`, y `equityWeight`/`debtWeight` no es uno de ellos). **No debe describirse la Decisión 2 como completamente implementada** — la política de producto está confirmada; su aplicación en el demo queda como deuda documentada, no como una tarea nueva (ver fila "Pendiente registrado" abajo, sin duplicar `BL-XX` existente) |
| Comportamiento permitido | Comportamiento del flujo profesional, sin cambios: `equityWeight`/`debtWeight` se derivan de saldos reales. El demo/estimador queda fuera de esta garantía hasta que se retire o se decida explícitamente mantener ese control ahí |
| Comportamiento prohibido | No se implementan overrides ni se modifica la estructura de capital en este bloque; tampoco se retiró el control del demo — ambas cosas fuera de alcance de `c6619d5` |
| Pendiente registrado | Retirar (o justificar explícitamente por qué se conserva) el control de `equityWeight` en `DemoInputsForm.tsx` — no autorizado en `c6619d5`, queda para una futura sesión con alcance explícito sobre la Decisión 2, coordinada con el criterio final del revisor sobre overrides de estructura de capital |
...
| Alcance real verificado (2026-08-06) | La validación protege exclusivamente el **demo/estimador** (`DemoInputsForm.tsx`, montado solo dentro de `DemoDashboard.tsx`, ejecuta el **motor cliente** `runAnalysis`). **No** es una modificación del flujo profesional real: `NewAnalysisStepper.tsx` (el flujo que crea un análisis pagado) no tiene ningún campo manual de ingresos — los datos financieros llegan exclusivamente vía documentos parseados por el pipeline del servidor. Confirmado por `grep`: `DemoInputsForm` no se importa en ningún otro componente |
...
| Decisión futura (10B, diferida) | El motor cliente (`runAnalysis`) también deberá rechazar `revenue = 0` como segunda capa de defensa. Confirmado por `grep`: `runAnalysis` tiene exactamente **2 llamadores reales** — `DemoDashboard.tsx` (el mismo demo, ya protegido por 10A) y `src/pages/Dashboard.tsx::handleDownloadPDF` (regenera el PDF client-side a partir del `input_payload` ya persistido por el pipeline del servidor, no de una entrada manual — arquitectura ya documentada como deuda en `D-06`, no creada por esta decisión). 10B revisa ambos antes de implementarse — no implementado en este bloque |
