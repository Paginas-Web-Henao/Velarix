...
| 5 | El decalaje demuestra que "EBITDA sin decalaje" no es automáticamente el EBITDA correcto ni proyectable | Sí | El Expediente V1 ya exige que reportado/normalizado/proyectado coexistan (§5, §17, confirmado por Caso 01) | Ninguno — se registra como caso de uso concreto que confirma la regla ya incorporada, no como estructura adicional | El decalaje es evidencia independiente (distinta de working capital de Tecnoglass) de la misma necesidad ya cubierta | `CASO-02-TERPEL-V0.md` §7 | Baja (no genera cambio estructural) | No aplica | No | No | `CONFIRMADO POR DOS CASOS` (ya cubierto, sin cambio) |
