...
> **Actualizado 2026-08-05 (1D-OPS — CERRADO)**: con el proyecto Supabase
> de desarrollo autenticado y vinculado, la migración se aplicó y ambas
> Edge Functions (`ejecutar-calculo`, `continuar-tras-revision`) quedaron
> desplegadas y `ACTIVE`, con autenticación mixta implementada
> explícitamente en el código (`verify_jwt: false`, justificado porque la
> puerta de entrada de Supabase no reconoce el formato de secret key
> nuevo) y migradas por completo a `SUPABASE_SECRET_KEYS`/
> `SUPABASE_PUBLISHABLE_KEYS` tras el incidente de exposición de la
> legacy `service_role`. R-09/`BL-10` verificado con 15 pruebas SQL/RPC/RLS
...
> precisada 2026-08-10 tras corrección del fundador)**: 1E gana una
> condición de entrada adicional a las tres de abajo — **no** la
> implementación completa del Expediente de Valoración, sino un
> **checkpoint mínimo operativo y validado**: debe existir evidencia,
> sobre al menos un caso, de que el contexto específico de una empresa
> puede representarse antes del cálculo (ver
> `docs/velarix/plan/EXPEDIENTE-DE-VALORACION-V1.md` y
> `docs/velarix/plan/REORIENTACION-BOUTIQUE-EXPERT-LED-2026-08-10.md`). No
> cambia el estado ya alcanzado de 1A/1B/1C/1D — **1D ya está cerrado**,
> no es un pendiente actual; los pendientes reales de 1B/1C siguen siendo
> metodológicos (revisor financiero externo).

- 1E depende de **1B, 1C y 1D cerrados** — no de "en progreso", cerrados.
- **(Nueva, 2026-08-10, precisada el mismo día)** 1E depende también de un
  checkpoint mínimo operativo y validado del Expediente de Valoración
  (no de su implementación completa) — conectar narrativa/PDF a un motor
  sin ninguna evidencia de que el contexto específico de una empresa
  puede representarse repetiría el problema que motivó la reorientación
  de negocio.
...
| Bloque 1E (integración y activación) | **1D ya está cerrado — no es un pendiente actual** (1D-OPS, 2026-08-05: migración aplicada, ambas Edge Functions desplegadas con autenticación mixta explícita y verificadas por HTTP real, ya migradas de la legacy `service_role`/`anon` a `SUPABASE_SECRET_KEYS`/`SUPABASE_PUBLISHABLE_KEYS`, y primer administrador real configurado, ver `docs/velarix/bloque-1d/REPORTE-ACTIVACION-1D.md`). Pendientes reales: 1B-metodología y 1C sin cerrar formalmente (dependen del revisor financiero externo). El inicio de 1E sigue siendo una decisión explícita del fundador — no se ha iniciado en ninguna sesión hasta ahora. **Actualizado 2026-08-10 (`D-09`, precisado el mismo día)**: además de 1B/1C cerrados, ahora también requiere un **checkpoint mínimo operativo y validado** del Expediente de Valoración (no su implementación completa) — ver `docs/velarix/plan/EXPEDIENTE-DE-VALORACION-V1.md` |
| Expediente de Valoración V1 — implementación (nuevo bloque, `D-09`, 2026-08-10) | Ninguna dependencia técnica previa — puede iniciarse en cualquier momento, pero **requiere autorización explícita y separada del fundador** (no autorizada en esta sesión, solo especificada). Se beneficia de, pero no depende estrictamente de, el estudio manual de casos (fila siguiente), que ya está en curso y sirve para refinar la especificación antes de implementar. Ver `docs/velarix/plan/EXPEDIENTE-DE-VALORACION-V1.md` |
| Estudio manual de casos públicos — Biblioteca (A) (`D-09`, precisado 2026-08-10) | **Ninguna — activo desde ahora.** No depende de la implementación del Expediente V1, ni del revisor financiero externo, ni de ningún bloque técnico. Primer caso: Tecnoglass (análisis manual, sin scraping ni automatización) — ver `docs/velarix/plan/EXPEDIENTE-DE-VALORACION-V1.md` |
| Biblioteca automatizada de casos (B) (iniciativa diferida, `D-09`, precisado 2026-08-10) | Sin dependencia técnica fijada — diferida por decisión de producto (ingestión automática, interfaz, scraping, procesamiento masivo), no por bloqueo técnico específico |
