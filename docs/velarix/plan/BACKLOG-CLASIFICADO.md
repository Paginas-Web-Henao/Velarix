...
> **Actualizado 2026-07-30 (Bloque 1D-OPS — CERRADO)**: `BL-10` queda
> **aplicado y verificado** en el proyecto Supabase de desarrollo (vacío,
> sin datos reales) — la brecha de auto-escalamiento ya no existe en ese
> entorno. `ejecutar-calculo` y `continuar-tras-revision` quedaron
> **desplegadas** (`ACTIVE`, `verify_jwt: true`) — el `403` inicial no era
> un límite de permisos, era un Project Ref equivocado (corregido en
> `.env`). 15 pruebas SQL/RPC/RLS + 14 pruebas HTTP autenticadas (con
> usuarios sintéticos reales) pasaron. **1D queda cerrado**. Pendiente:
> configurar un primer administrador real antes de 1E (la cuenta de
> prueba se eliminó al cerrar). Ver
